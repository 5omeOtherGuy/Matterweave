...
**Owner requirement**: an explicit current goal. **Working requirement**: an engineering interpretation or setup convention the implementation session may refine with evidence. **Research candidate**: an opportunity, not a delivery commitment. All acceptance evidence below is outstanding unless STATUS.md says otherwise.
...
| R10 | Provide usable mobile controls. | Working requirement, consistent with Android goal and earlier explicit request | Simultaneous move/look/action, cancel/focus-loss handling, adjustable layout, no desktop-only actions. | 0015, 0011 / M0–M1 |
| R11 | Reuse viable alternatives meeting strict criteria instead of unnecessary custom implementation. | Owner requirement, strengthened after language discussion | Candidate/source/license/version assessment; measured fit and justified adopt/adapt/build outcome. | 0014 / all |
...
| R17 | Use Rust wherever feasible without material detriment to project goals. | Owner requirement | Rust core and suitable tools/components; documented, narrow other-language exceptions. | 0014, 0015 / all |
| R18 | Make engine subsystems modular while preserving maximum useful speed and efficiency. | Owner requirement | Explicit ownership/data contracts, independent tests, replaceable backend boundaries and measured overhead. | 0014, 0015 / M0 onward |
| R19 | Create technology, tools and Rust hardware interfaces when necessary or significantly advantageous. | Owner requirement | Existing-solution assessment, gap/hypothesis, bounded prototype and evidence before production adoption. | 0014, 0015 / when justified |
| R20 | Consider Jolt only when it provides major advantages over viable Rust alternatives. | Owner requirement | Workload-relevant capability/quality/performance evidence including bindings, transfers, build and maintenance costs. | 0010, 0014 / physics selection |
...
There is no accepted exact FPS target, RAM allowance, voxel size, world size, device floor or UE5 parity claim. Proposed evaluation budgets are in BENCHMARKS.md, where they are explicitly identified as working targets.

Rust preference, modularity and reuse are accepted selection constraints. Significant or major advantage is assessed against a workload-specific criterion established before an experiment, not an invented universal percentage. See component selection; missing bindings do not automatically justify switching language, and Rust preference does not justify rewriting adequate tools solely for purity.
