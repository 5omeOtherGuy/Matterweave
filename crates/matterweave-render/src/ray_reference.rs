...
pub const VERTEX_SPIRV: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/ray_reference.vs_main.spv"));
pub const FRAGMENT_SPIRV: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/ray_reference.fs_main.spv"));
...
impl RayVolume {
    ...
    pub fn pack(
        ...
        let cells = dimensions
            ...
            .ok_or("Ray volume exceeds 64^3 cells")?;
        for axis in 0..3 {
        ...
        materials
            ...
            .map_err(|e| format!("Ray allocation: {e}"))?;
    ...
    pub fn memory_stats(&self) -> RayMemoryStats {
        ...
        RayMemoryStats {
            ...
            upload_bytes: material_bytes + PALETTE_BYTES + UNIFORM_BYTES,
        }
    }
    ...
    pub fn uniform(
        ...
        if !inverse.is_finite() {
            return Err("Ray inverse camera is nonfinite".into());
        }
        Ok(RayUniform {
...
mod tests {
    ...
    // These exercise the existing CPU oracle, NOT execution of the shader.
    // They define lead-owned GPU readback expectations without a duplicate DDA.
    #[test]
    fn oracle_negative_boundary_parallel_and_inside_contract() {
        let mut world = World::new(0);
        world.set([-17, 0, 0], 255);
        let hit = world
            .raycast([-16.0, 0.5, 0.5], [-1.0, 0.0, 0.0], 3.0)
            .unwrap();
        assert_eq!(hit.cell, [-17, 0, 0]);
        assert_eq!(hit.distance, 0.0);
        assert_eq!(hit.normal, [1, 0, 0]);
        let inside = world
            .raycast([-16.5, 0.5, 0.5], [1.0, 0.0, 0.0], 3.0)
            .unwrap();
        assert_eq!(inside.normal, [0; 3]);
        assert_eq!(inside.distance, 0.0);
        assert!(world
            .raycast([-18.0, 1.0, 0.5], [1.0, 0.0, 0.0], 3.0)
            .is_none());
        let pack = RayVolume::pack(&world, 0, [-17, 0, 0], [1; 3], palette()).unwrap();
        assert_eq!(pack.materials(), &[u32::from(hit.material)]);
    }

    #[test]
    fn oracle_crop_entry_ties_internal_negative_grid_plane() {
        let mut world = World::new(0);
        world.set([0, 0, 0], 9); // Only touched at the entry corner: skip.
        world.set([-1, 0, 0], 10);
        let hit = world
            .raycast([0.5, -0.5, 0.5], [-1.0, 1.0, 0.0], 3.0)
            .unwrap();
        assert_eq!(hit.cell, [-1, 0, 0]);
        assert_eq!(hit.normal, [1, 0, 0]);
        let pack = RayVolume::pack(&world, 0, [-1, 0, 0], [2, 1, 1], palette()).unwrap();
        assert_eq!(pack.materials(), &[10, 9]);
    }

    #[test]
    fn oracle_ties_step_together_and_crop_is_not_whole_world() {
        let mut world = World::new(0);
        world.set([-2, -3, -3], 1); // Merely edge-touched: must skip.
        world.set([-2, -2, -2], 2);
        let hit = world.raycast([-2.5; 3], [1.0; 3], 4.0).unwrap();
        assert_eq!(hit.cell, [-2; 3]);
        assert_eq!(hit.normal, [-1, 0, 0]);
        assert!((hit.distance - 0.5 * 3.0_f32.sqrt()).abs() < 1e-6);
        let pack = RayVolume::pack(&world, 0, [-2; 3], [1; 3], palette()).unwrap();
        assert_eq!(pack.materials(), &[2]);
        // A CPU whole-world query sees this excluded foreground voxel first.
        world.set([-4; 3], 7);
        assert_eq!(
            world.raycast([-4.5; 3], [1.0; 3], 10.0).unwrap().material,
            7
        );
        assert_eq!(pack.materials(), &[2]);
        // Documented difference: CPU can hit a lower-face outward origin at t=0;
        // the shader rejects zero-length AABB overlap before checking any cell.
        let outward = world
            .raycast([-2.0, -1.5, -1.5], [-1.0, 0.0, 0.0], 1.0)
            .unwrap();
        assert_eq!(outward.cell, [-2; 3]);
        assert_eq!(outward.normal, [0; 3]);
    }
    ...
    fn uniform_layout_and_camera_round_trip() {
        ...
        for proj in [
            Mat4::perspective_rh(1.0, 1.4, 0.1, 100.0),
        ...
        assert!(pack
            .uniform(Mat4::ZERO, eye, crate::Sun::default())
            .is_err());
        assert!(pack
            .uniform(Mat4::IDENTITY, Vec3::NAN, crate::Sun::default())
            .is_err());
        let sun = crate::Sun {
            direction_to_sun: [0.0; 3],
            ..Default::default()
        };
        assert!(pack.uniform(Mat4::IDENTITY, eye, sun).is_err());
