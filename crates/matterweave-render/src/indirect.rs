...
struct Key { epoch: u64, revision: u64, sun: [f32;4] }
fn light_key(sun: Sun) -> Result<[f32;4], String> {
    let d=Vec3::from_array(sun.direction_to_sun);
    if !d.is_finite() || !d.length_squared().is_finite() || d.length_squared()<1e-12
    ...
    let n=d.normalize(); Ok([n.x,n.y,n.z,sun.intensity])
}
...
impl IndirectVolume {
    pub fn new(origin: [i32;3], dimensions: [u32;3], samples: u32, distance: f32,
        palette: [[f32;3];256]) -> Result<Self,String> {
        let slots=dimensions.iter().try_fold(6usize,|n,&d| n.checked_mul(d as usize))
            .filter(|&n| n>0 && n<=MAX_FACE_SLOTS).ok_or("Indirect volume exceeds face residency cap")?;
        if !(1..=256).contains(&samples) || !distance.is_finite() || !(0.001..=256.).contains(&distance)
            || palette.iter().flatten().any(|&c| !c.is_finite() || !(0.0..=1.0).contains(&c)) {
        ...
        for axis in 0..3 {
            ...
            if origin[axis] < -8192 || end>8192 { return Err("Indirect bounds exceed precise voxel coordinate range".into()); }
        }
        let mut values=Vec::new();
        values.try_reserve_exact(slots).map_err(|e|format!("Indirect allocation: {e}"))?;
        values.resize(slots,[0.;4]);
        Ok(Self {origin,dimensions,values,palette,samples,distance,key:None,cursor:0,sample_index:0,sum:Vec3::ZERO})
    }
    pub fn resident_bytes(&self) -> usize { self.values.len()*16 + std::mem::size_of::<Self>() }
    pub fn valid_for(&self, world: &World, epoch: u64, sun: Sun) -> bool {
        light_key(sun).is_ok_and(|sun|self.key==Some(Key {epoch,revision:world.revision(),sun}))
    }
    pub(crate) fn sun_matches(&self, sun: Sun) -> bool {
        light_key(sun).is_ok_and(|s|self.key.is_some_and(|k|k.sun==s))
    }
    pub(crate) fn cached_sun(&self) -> Option<[f32;4]> { self.key.map(|k|k.sun) }
    pub fn sample(&self, cell: [i32;3], face: usize) -> [f32;3] {
        if face>=6 { return [0.;3]; }
        let c: [i64;3]=std::array::from_fn(|a|i64::from(cell[a])-i64::from(self.origin[a]));
        if (0..3).any(|a|c[a]<0 || c[a]>=i64::from(self.dimensions[a])) {return [0.;3];}
        let i=((c[2] as usize*self.dimensions[1] as usize+c[1] as usize)*self.dimensions[0] as usize+c[0] as usize)*6+face;
        let v=self.values[i]; [v[0],v[1],v[2]]
    }
    ...
    pub fn update(&mut self, world: &World, epoch: u64, sun: Sun, budget: UpdateBudget) -> Result<UpdateStats,String> {
        let light=light_key(sun);
        let key=light.as_ref().ok().map(|&sun|Key {epoch,revision:world.revision(),sun});
        if key!=self.key || key.is_none() {
            self.values.fill([0.;4]); self.cursor=0; self.sample_index=0; self.sum=Vec3::ZERO; self.key=key;
        }
        let light=light?;
        ...
        while self.cursor<self.values.len() && stats.work<budget.work.min(MAX_UPDATE_WORK) {
            ...
            let dx=self.dimensions[0] as usize; let dy=self.dimensions[1] as usize;
            let cell=[self.origin[0]+(index%dx) as i32,self.origin[1]+((index/dx)%dy) as i32,self.origin[2]+(index/(dx*dy)) as i32];
            let normal=NORMALS[face];
            let neighbor=std::array::from_fn(|a|cell[a]+normal[a]);
            if world.get(cell)==0 || world.get(neighbor)!=0 {self.cursor+=1;continue;}
            if stats.rays+2>ray_cap {break;}
            let n=Vec3::from_array(normal.map(|n|n as f32));
            let origin=Vec3::from_array(cell.map(|v|v as f32))+Vec3::splat(0.5)+n*(0.5+EPSILON);
            ...
            if let Some(hit)=world.raycast(origin.to_array(),ray.to_array(),self.distance) {
                ...
                if cosine>0. {
                    ...
                    stats.rays+=1;
                    if world.raycast(point.to_array(),direction.to_array(),self.distance).is_none() {
                        self.sum+=Vec3::from_array(self.palette[hit.material as usize])*(light[3]*cosine);
                    }
                }
            }
            self.sample_index+=1;
            if self.sample_index==self.samples {
                ...
                self.cursor+=1;self.sample_index=0;self.sum=Vec3::ZERO;
            }
        }
        stats.complete=self.cursor==self.values.len(); Ok(stats)
    }
}
