...
fn full_pool_reuse_waits_for_unload_and_stale_handles_cannot_touch_replacement() {
    ...
    assert_eq!(frame(&mut service), [0.0; 2], "old PCM must stop before reuse");
    let replacement_samples = vec![-0.125; MAX_PCM_BYTES / 4];
    let replacement = service.register_clip(ClipSpec::mono(&replacement_samples)).unwrap();
    let replacement_voice = service.play(replacement, PlayOptions::default()).unwrap();
    assert_eq!(service.unregister_clip(old_clip), Err(AudioServiceError::StaleClipHandle));
    assert_eq!(service.play(old_clip, PlayOptions::default()), Err(AudioServiceError::StaleClipHandle));
