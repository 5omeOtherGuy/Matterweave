...
fn failed_start_error_must_not_disconnect_healthy_replacement() {
    ...
    assert_eq!(service.resume(), Err(AudioServiceError::StreamStartFailed { code: -1 }));
    service.resume().unwrap();
    let recreated = service.health().stream_recreations;
    service.poll_device().unwrap();
    assert_eq!(service.health().stream_recreations, recreated,
...
fn recreation_clears_old_error_before_open_but_preserves_new_stream_error() {
    let mut service = playing();
    service.mock_backend().unwrap().inject_device_error(-4);
    service.recreate_output_with(|core, shared| {
        assert!(!shared.disconnected.load(Ordering::Acquire), "old error survives into new open");
    ...
    assert_eq!(service.shared.error_code.load(Ordering::Acquire), -5,
    ...
    assert_eq!(service.health().stream_recreations, 2);
