...
fn short_waterside_itinerary_is_a_recorded_contiguous_part_of_the_walked_loop() {
    ...
    let length: f32 = waterside
        .windows(2)
        .map(|p| (p[1][0] - p[0][0]).hypot(p[1][2] - p[0][2]))
        .sum();
    assert!(
        (20.0..=120.0).contains(&length),
        "waterside length {length}"
    );
    assert!(
        waterside.iter().any(|p| map
            .terrain
            .surface_at_metres(p[0], p[2])
    ...
    assert!(
        (end[0] - 42.0).hypot(end[2] - 90.0) < 5.0,
